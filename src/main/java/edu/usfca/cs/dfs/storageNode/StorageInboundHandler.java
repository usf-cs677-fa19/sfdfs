...
public class StorageInboundHandler extends InboundHandler {
    ...
    public void channelRead0(ChannelHandlerContext ctx, StorageMessages.StorageMessageWrapper msg) {
        ...
        if(msg.hasStoreChunkMsg()) {
            StorageNodeDS.getInstance().logger.log(Level.INFO,"\n**************************storage receieved store chunk *******************************\n");
           StorageNodeDS.getInstance().logger.log(Level.INFO,"Size of storage node list : "+ msg.getStoreChunkMsg().getStorageNodeIdsList().size());
            ...
            if(isMetaWritten && isChunkWritten) {
                ...
                StorageNodeDS.getInstance().logger.log(Level.INFO,"Meta and Chunk saved on Storage node :->");
            } else {
            ...
            StorageNodeDS.getInstance().logger.log(Level.INFO,"RetrieveChunkMeta received from controller");
            ...
            StorageNodeDS.getInstance().logger.log(Level.INFO,"Sent chunkMetaInfo Back to the controller");
           // ctx.close();
        }
        else if(msg.hasRetrieveChunkMsg()) {  //storage node should send chunkMsg
            StorageNodeDS.getInstance().logger.log(Level.INFO,"Client asking for a file chunk");
                ...
                if(Fileify.doesFileExist(pathForFileChunkId)) {
                    try {
                        ...
                        if(checksumNew == checksumExisting) {
                            StorageNodeDS.getInstance().logger.log(Level.INFO,"Checksum matches....... :) :) ");
                            ...
                            StorageNodeDS.getInstance().logger.log(Level.INFO,"Sending fileChunk to client : "+ msgWrapper.getChunkMsg().getFileChunkId());
                            ...
                            StorageNodeDS.getInstance().logger.log(Level.INFO,"Checksum does not match :( :(");
                            ...
                            StorageNodeDS.getInstance().logger.log(Level.INFO,"Preparing  and sending BadChunkFoundMsg  to controller");
            ...
            StorageNodeDS.getInstance().logger.log(Level.INFO,"Storgage asking for a file chunk to replace bad chunk");
            boolean isChunkFound = false;
            ByteBuffer buff;
...
//                            StorageMessages.StorageMessageWrapper msgWrapper = StorageStorageMessagesHelper.prepareChunkMsg(fileChunkId, ByteBuffer.wrap(arr));
                            ...
                            StorageNodeDS.getInstance().logger.log(Level.INFO,"Sending fileChunk to storage node with primaryId folder  : "
...
//            }
            ...
            StorageNodeDS.getInstance().logger.log(Level.INFO,"Become Primary!!!");
            ...
            StorageNodeDS.getInstance().logger.log(Level.INFO,"Creating new replica at : "+newReplicaID);
            ...
            StorageNodeDS.getInstance().logger.log(Level.INFO,"Heal bad chunk received from controller");
            ...
            StorageNodeDS.getInstance().logger.log(Level.INFO,"Connecting Info for hasHealBadChunkMsg : ");
            for(int i =0; i<msg.getHealBadChunkMsg().getStorageNodesCount(); i++) {
                ...
                try {
                    ...
                    if(f.isSuccess() && StorageNodeDS.getInstance().getHealed(msg.getHealBadChunkMsg().getBadFileChunkId()) == 1) {
                        StorageNodeDS.getInstance().logger.log(Level.INFO,"is Success in hasHealBadChunkMsg, for i = "+i);
                        ctx.close();
                        break;
                    }
            ...
            if(msg.getChunkForBadChunkMsg().getFound() == true) {
                ...
                StorageNodeDS.getInstance().logger.log(Level.INFO,"CHUNK NOT FOUND MSG : ");
                StorageNodeDS.getInstance().logger.log(Level.INFO,"Storage node Ids : ");
                ...
                StorageNodeDS.getInstance().logger.log(Level.INFO,"Chunk not found message");
            }
        }
    }
    ...
    private void sendChunksToReplica(String nodeId,String sourcePath,List<String> storageNodes){
        ...
        if(source.isDirectory()){
            ...
            for(File file : files) {
                ...
                StorageNodeDS.getInstance().logger.log(Level.INFO,"Filename : "+filepaths[filepaths.length-1]);
                ...
                try {
                   ...
                   new MessageSender().send(false,"storage",connectingIPAddressAndPort[0],Integer.parseInt(connectingIPAddressAndPort[1]),msgWrapper);
                    ...
                    StorageNodeDS.getInstance().logger.log(Level.SEVERE,"Exception while sending chunks to replicas of the new primary!!");
                    e.printStackTrace();
                }
            }
        }
...
//        }
    ...
    private StorageMessages.StorageMessageWrapper handleChunkNotFound(String fileChunkId, StorageMessages.StorageMessageWrapper msg){
        StorageNodeDS.getInstance().logger.log(Level.INFO,"Not found : "+fileChunkId+" in any directory, sending not found message");
        ...
        for(int i = 0; i< msg.getRetrieveChunkMsg().getStorageNodeIdsCount(); i++) {
            StorageNodeDS.getInstance().logger.log(Level.INFO,"checking from list : "+ msg.getRetrieveChunkMsg().getStorageNodeIds(i));
            if(msg.getRetrieveChunkMsg().getStorageNodeIds(i).equalsIgnoreCase(selfId)) {
                StorageNodeDS.getInstance().logger.log(Level.INFO," - - Removing self from list : "+ selfId);
            } else {
